...
namespace Services.Web
{
    public static class HttpClientHelper
    {
        ...
        private static ResponseException CreateException(UnityWebRequest request)=>
            new ResponseException(request.result.ToString(),request.error);
        ...
        private static bool TryDeserializeJson<T>(string json, out T data)
        {
            ...
            {
                Debug.LogError($"[{nameof(HttpClientHelper)}:{nameof(TryDeserializeJson)}]: Fail deserialize to '{typeof(T)}' " +
