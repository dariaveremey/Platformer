...
{
    public class UnityHttpWebRequestFactory : MonoBehaviour, IHttpRequestFactory
    {
        ...
        {
            Response response = HttpClientHelper.ConvertResponse(request);
            responseCallback?.Invoke(response);
        }

        private void HandleHttpResponse<TDto>(UnityWebRequest request, Action<DtoResponse<TDto>> responseCallback)
        {
            DtoResponse<TDto> response = HttpClientHelper.ConvertResponse<TDto>(request);
            responseCallback?.Invoke(response);
        }
    }
}